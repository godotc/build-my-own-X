...
enum class EVimEditorMode : uint8_t {
  Normal = BIT(0),
...
inline bool is_cursor_controlled(EVimEditorMode mode) {
  constexpr auto flags = (uint8_t)EVimEditorMode::Insert |
                         (uint8_t)EVimEditorMode::Terminal |
                         (uint8_t)EVimEditorMode::Ex;
  return (uint8_t)mode & (~flags);
}

template <class T> String EnumToString(T) = delete;
inline String EnumToString(EVimEditorMode mode) {
  switch (mode) {
